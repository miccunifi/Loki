...
//		}
	...
	load: function(url, settings) {
		...
		var loadSVG = function(data) {
			...
			//Removing empty paramethers
			var parent = data.getElementsByTagName('g')[0];
			if(!$(parent).attr('id')){
				var paths = $(data).find('path[d=""]');
				for(var k = 0; k < paths.length; k++){
					var nodeParent = $(paths[k]).parent();
					$(paths[k]).remove();
				}
			}
			
			parent = (settings.parent ? $(settings.parent)[0] : wrapper._svg);
			var attrs = {};
		...
		if (url.match('<svg')) { // Inline SVG
			loadSVG( /MSIE/.test(navigator.userAgent) ? loadXML4IE(url) :
				new DOMParser().parseFromString(url, 'text/xml'));
		}
		else { // Remote SVG
			$.ajax({url: im3iSoapCalls.unifiProxy + url, dataType: ( /MSIE/.test(navigator.userAgent) ? 'text' : 'xml'),
				success: function(xml) {
					loadSVG( /MSIE/.test(navigator.userAgent)? loadXML4IE(xml) : xml);
