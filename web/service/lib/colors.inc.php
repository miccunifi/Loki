		...
		if (isset($this->image))
		{
			...
			for ($y=0; $y < $imgHeight; $y++)
			{
				for ($x=0; $x < $imgWidth; $x++)
				{
					...
					$Colors['red']=intval((($Colors['red'])+64)/128)*128;    //ROUND THE COLORS, TO REDUCE THE NUMBER OF COLORS, SO THE WON'T BE ANY NEARLY DUPLICATE COLORS!
					$Colors['green']=intval((($Colors['green'])+64)/128)*128;
					$Colors['blue']=intval((($Colors['blue'])+64)/128)*128;
					if ($Colors['red']>=256)
					$Colors['red']=255;
					if ($Colors['green']>=256)
					$Colors['green']=255;
